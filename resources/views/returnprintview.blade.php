...
<body>
        ...
        <table id="customers">
            ...
            <tbody>
                @php
                    $total = 0;
                @endphp
                @foreach ($returnprintArr as $returnproduct)
                    <tr>
                        <td>{{ $returnproduct->rproduct }}</td>
                        <td>{{ $returnproduct->rquantity }}</td>
                        <td>{{ $returnproduct->rprice }}</td>
                        <td>{{ $returnproduct->rquantity * $returnproduct->rprice }}
                            @php
                                $total += $returnproduct->rquantity * $returnproduct->rprice;
                            @endphp
                        </td>
                    </tr>
                @endforeach
    ...
    @php
        $curr = date('h : i A');
    @endphp
    <h5 class="kkk"><strong id="serialnumber">Serial Number : <span id="result"></span></strong></h5>
